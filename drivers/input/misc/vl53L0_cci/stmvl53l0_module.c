...
static struct stmvl53l0_data *pdata = NULL;
/* MM-AL-RangeFinderPorting-01*} */

int stmvl53l0_stop_access_on_bus_error = 0;
...
*/
#define VL53L0_IOCTL_SETOFFSET		_IOW('p', 0x07, int32_t)
/* MM-AL-RangeFinderPorting-01*} */
...
#define CALIB_DATA_LEN   8
/* MM-AL-AddSEPolicyForCalibLoad-00+} */
/* int8_t offset_calib; */
int32_t offset_calib;
int16_t xtalk_calib;
int8_t is_offset_valid = 0;
int8_t is_xtalk_valid = 0;
#endif
/* MM-AL-RangeFinderPorting-01*} */
...
*/
		...
		vl53l0_dbgmsg("flip_open err=%d\n", IS_ERR(f));
	}
#else
	/* MM-AL-FixCalibrationError-00*{ */
	if (is_offset_valid && is_xtalk_valid) {
		vl53l0_errmsg("Set offset calibration data(%d)\n", offset_calib);
...
*/
	...
	/* MM-AL-FixCalibrationError-00*{ */
	if (is_offset_valid && is_xtalk_valid) {
		vl53l0_errmsg("Set xtalk calibration data(%d)!\n", xtalk_calib);
...
static void stmvl53l0_write_offset_calibration_file(void)
{
	...
	if (f != NULL && !IS_ERR(f)) {
		fs = get_fs();
		set_fs(get_ds());
		snprintf(buf, PAGE_SIZE, "%d", offset_calib);
...
static void stmvl53l0_write_xtalk_calibration_file(void)
{
	...
	if (f != NULL && !IS_ERR(f)) {
		fs = get_fs();
		set_fs(get_ds());
		snprintf(buf, PAGE_SIZE, "%d", xtalk_calib);
...
/* MM-JF-ReturnRangerfinderRrrorCodeWhenInitFail-00*{ */
//static void stmvl53l0_setupAPIFunctions(struct stmvl53l0_data *data)
static VL53L0_Error stmvl53l0_setupAPIFunctions(struct stmvl53l0_data *data)
{
	uint8_t revision = 0;
	VL53L0_DEV vl53l0_dev = data;
	VL53L0_Error err = VL53L0_ERROR_NONE;

	/* Read Revision ID */
	err = VL53L0_RdByte(vl53l0_dev, VL53L0_REG_IDENTIFICATION_REVISION_ID, &revision);
...
static void stmvl53l0_ps_read_fake_measurement(struct stmvl53l0_data *data)
{
	...
	input_report_abs(data->input_dev_ps, ABS_HAT0X, tv.tv_sec);
	input_report_abs(data->input_dev_ps, ABS_HAT0Y, tv.tv_usec);
	input_report_abs(data->input_dev_ps, ABS_HAT1X, 8191);
	input_report_abs(data->input_dev_ps, ABS_HAT1Y, 0x4);
	input_report_abs(data->input_dev_ps, ABS_HAT2X, 14336);
	...
	//}
	input_report_abs(data->input_dev_ps, ABS_MISC, 47876);
	input_sync(data->input_dev_ps);

	if (data->enableDebug)
		vl53l0_errmsg(
"[fake] range:%d, RtnRateMcps:%d,err:0x%x,Dmax:%d,rtnambr:%d,time:%d,"
	"Spad:%d,SigmaLimit:%d\n",
			data->rangeData.RangeMilliMeter,
			data->rangeData.SignalRateRtnMegaCps,
			data->rangeData.RangeStatus,
...
int stmvl53l0_poll_thread(void *data)
{
	...
	while(!kthread_should_stop()) {
		...
		/* MM-MC-Fix-TOF-deinit-deadlock-01+{ */
		if (stmvl53l0_stop_access_on_bus_error)
		{
			pr_err("%s(%d) : stmvl53l0_stop_access_on_bus_error = %d, Send fake TOF data \n", __FUNCTION__ , __LINE__
                                                                                            , stmvl53l0_stop_access_on_bus_error);
			stmvl53l0_ps_read_fake_measurement(vl53l0_dev);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (!pdata) {
		vl53l0_errmsg("drv data pointer does NOT exist\n");
		return snprintf(buf, PAGE_SIZE, "(ERROR: NULL data pointer)\n");
	} else {
		data = pdata;
	}
	/* MM-AL-RangeFinderPorting-01*} */
				...
				size_t count)
{
	...
	if ((val != 0) && (val != 1)) {
        ...
        if (val == 9)
        {
            struct cci_data *cci_data = (struct cci_data *)data->client_object;
            cci_data->power_up = 0;
            vl53l0_errmsg("val = %ld, set cci_data->power_up = 0 for debug !!\n", val);
        }
        /* MM-MC-Fix-TOF-deinit-deadlock-00+} */
        /* MM-MC-Fix-TOF-deinit-deadlock-01+{ */
        if (val == 8)
        {
            vl53l0_errmsg("val = %ld, set stmvl53l0_stop_access_on_bus_error = 1 for debug !!\n", val);
            stmvl53l0_stop_access_on_bus_error = 1;
        }
        /* MM-MC-Fix-TOF-deinit-deadlock-01+} */
		vl53l0_errmsg("store invalid value=%ld\n", val);
		return count;
	}
	/* MM-AL-RangeFinderPorting-01*} */
	...
	if (val == 1) {
		...
		/* MM-MC-Fix-TOF-deinit-deadlock-00+{ */
		//Re-seset for MSM_SD_SHUTDOWN
		struct cci_data *cci_data = (struct cci_data *)data->client_object;
		if (cci_data->power_up == 0)
		{
...
static DEVICE_ATTR(show_distance, 0660,
                                        stmvl53l0_show_distance, NULL);
/* MM-AL-AddForFQC-00+} */

/* MM-AL-RangeFinderPorting-01*{ */
#ifdef CALIBRATION_FILE
//data for offset calibration
char string_invalid[] = "No valid data";
static ssize_t stmvl53l0_show_cal_offset(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	//struct stmvl53l0_data *data = dev_get_drvdata(dev);
...
static ssize_t stmvl53l0_store_cal_offset(struct device *dev,
					struct device_attribute *attr, const
					char *buf, size_t count)
{
	//struct stmvl53l0_data *data = dev_get_drvdata(dev);
	...
	res = kstrtol(buf, 10, &offset_value);
	if (res != 0) {
		vl53l0_errmsg("err=%d with cal_offset string=%s\n", res, buf);
		return count;
	}
	is_offset_valid = 1;
	offset_calib = offset_value;

	return count;
}
/* DEVICE_ATTR(name,mode,show,store) */
static DEVICE_ATTR(cal_offset, 0660/*S_IWUSR | S_IRUGO*/,
				   stmvl53l0_show_cal_offset,
				   stmvl53l0_store_cal_offset);

//data for xtalk calibration
static ssize_t stmvl53l0_show_cal_xtalk(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	//struct stmvl53l0_data *data = dev_get_drvdata(dev);
...
static ssize_t stmvl53l0_store_cal_xtalk(struct device *dev,
					struct device_attribute *attr, const
					char *buf, size_t count)
{
	//struct stmvl53l0_data *data = dev_get_drvdata(dev);
	...
	res = kstrtol(buf, 10, &xtalk_value);
	if (res != 0) {
		vl53l0_errmsg("err=%d with cal_xtalk string=%s\n", res, buf);
		return count;
	}
	is_xtalk_valid = 1;
	xtalk_calib = xtalk_value;

	return count;
}
/* DEVICE_ATTR(name,mode,show,store) */
static DEVICE_ATTR(cal_xtalk, 0660/*S_IWUSR | S_IRUGO*/,
				   stmvl53l0_show_cal_xtalk,
				   stmvl53l0_store_cal_xtalk);

/* MM-AL-AddSEPolicyForCalibLoad-00+{ */
static ssize_t stmvl53l0_store_cal_data(struct device *dev,
					struct device_attribute *attr, const
					char *buf, size_t count)
{
	//struct stmvl53l0_data *data = dev_get_drvdata(dev);
	char offset_buf[CALIB_DATA_LEN+1], xtalk_buf[CALIB_DATA_LEN+1];
	long offset_value, xtalk_value;
	int res;

	if (count != 16) {
		vl53l0_errmsg("count size(%zu) is not 16, string=%s\n", count, buf);
		return count;
	}
	memcpy(&offset_buf[0], &buf[0], CALIB_DATA_LEN);
	offset_buf[CALIB_DATA_LEN]='\0';
	memcpy(&xtalk_buf[0], &buf[CALIB_DATA_LEN], CALIB_DATA_LEN);
	xtalk_buf[CALIB_DATA_LEN]='\0';

	res = kstrtol(offset_buf, 10, &offset_value);
	if (res != 0) {
		vl53l0_errmsg("err=%d with offset_value, string=%s\n", res, offset_buf);
		return count;
	} else {
		is_offset_valid = 1;
		offset_calib = offset_value;
	...
	res = kstrtol(xtalk_buf, 10, &xtalk_value);
	if (res != 0) {
		vl53l0_errmsg("err=%d with xtalk_value, string=%s\n", res, buf);
		return count;
	} else {
		is_xtalk_valid = 1;
		xtalk_calib = xtalk_value;
...
/* DEVICE_ATTR(name,mode,show,store) */
static DEVICE_ATTR(cal_data, 0220/*S_IWUSR | S_IRUGO*/,
				   NULL,
				   stmvl53l0_store_cal_data);
/* MM-AL-AddSEPolicyForCalibLoad-00+} */
#endif //#ifdef CALIBRATION_FILE
/* MM-AL-RangeFinderPorting-01*} */

/* for work handler scheduler time */
static ssize_t stmvl53l0_store_do_flush(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	//struct stmvl53l0_data *data = dev_get_drvdata(dev);
...
/* DEVICE_ATTR(name,mode,show,store) */
static DEVICE_ATTR(do_flush, 0220/*S_IWUGO | S_IRUGO*/,
				   NULL,
					stmvl53l0_store_do_flush);
			...
			void __user *p)
{
	...
	/* MM-AL-RangeFinderPorting-01*{ */
	int32_t offsetint = 0; /* int8_t offsetint = 0;*/
	/* MM-AL-RangeFinderPorting-01*} */
	struct stmvl53l0_data *data =
...
*/
	...
	case VL53L0_IOCTL_SETOFFSET:
		...
		/* MM-AL-RangeFinderPorting-01*{ */
		if (copy_from_user(&offsetint, (int32_t *)p, sizeof(int32_t))) {
			vl53l0_errmsg("%d, fail\n", __LINE__);
			return -EFAULT;
		}
...
static int stmvl53l0_init_client(struct stmvl53l0_data *data)
{
	...
	/* MM-JF-ReturnRangerfinderRrrorCodeWhenInitFail-00*{ */
	Status = stmvl53l0_setupAPIFunctions(data);
	if (Status != VL53L0_ERROR_NONE) {
		pr_err("Call of stmvl53l0_setupAPIFunctions return err(%d)\n", Status);
		return (int) Status;
	}
	/* MM-JF-ReturnRangerfinderRrrorCodeWhenInitFail-00*} */
	...
	/* MM-AL-RangeFinderPorting-01*{ */
	/* MM-AL-FixCalibrationError-00*{ */
	if (data->op_mode == NORMAL_MODE) {
		stmvl53l0_read_calibration_file(data);
	} else {
		vl53l0_dbgmsg("Not read calib. data in op mode(%d)\n", data->op_mode);
	}
	/* MM-AL-FixCalibrationError-00*} */
	/* MM-AL-RangeFinderPorting-01*} */
#endif
	...
	init_mode_e mode)
{
	...
	data->op_mode = mode; /* MM-AL-FixCalibrationError-00+ */
	...
	if (mode != NORMAL_MODE) {
		...
		if (is_xtalk_valid && is_offset_valid)
			papi_func_tbl->SetXTalkCompensationEnable(vl53l0_dev, 1);
		else
	...
	if (vl53l0_dev->useLongRange) {
		...
		Status = papi_func_tbl->SetLimitCheckValue(vl53l0_dev,
						VL53L0_CHECKENABLE_SIGNAL_RATE_FINAL_RANGE,
						(FixPoint1616_t)(25 * 65536 / 100)); ///fihtdc@20170824, peter patch ///(65536/10)); /* 0.1 * 65536 */
		if (Status == VL53L0_ERROR_NONE) {
			Status = papi_func_tbl->SetLimitCheckValue(vl53l0_dev,
						VL53L0_CHECKENABLE_SIGMA_FINAL_RANGE,
						(FixPoint1616_t)(32*65536));///fihtdc@20170824, peter patch ///(60*65536));
		} else {
		...
		if (Status == VL53L0_ERROR_NONE) {
			Status = papi_func_tbl->SetVcselPulsePeriod(vl53l0_dev,
						VL53L0_VCSEL_PERIOD_PRE_RANGE, 14);///fihtdc@20170824, peter patch ///18);
		} else {
		...
		if (Status == VL53L0_ERROR_NONE) {
			Status = papi_func_tbl->SetVcselPulsePeriod(vl53l0_dev,
				VL53L0_VCSEL_PERIOD_FINAL_RANGE, 10);///fihtdc@20170824, peter patch ///14);
		} else {
			pr_err("SetVcselPulsePeriod(PRE, 14) failed with errcode = %d\n", Status);///fihtdc@20170824, peter patch ///18) failed with errcode = %d\n", Status);
		}

		if (Status != VL53L0_ERROR_NONE) {
			pr_err("SetVcselPulsePeriod(FINAL, 10) failed with errcode = %d\n", Status);///fihtdc@20170824, peter patch ///14) failed with errcode = %d\n", Status);
		}
	} else {
...
static int stmvl53l0_stop(struct stmvl53l0_data *data)
{
	...
	/* clear flag of stop access */
	stmvl53l0_stop_access_on_bus_error = 0;
	/* power down */
...
static void stmvl53l0_timer_fn(unsigned long arg)
{

	//AA//VL53L0_DEV vl53l0_dev = (VL53L0_DEV)data;
	struct stmvl53l0_data *data;
	if (!pdata) {
		vl53l0_errmsg("drv data pointer does NOT exist\n");
		return;
	...
	input_report_abs(data->input_dev_ps, ABS_GAS,
		  data->flushCount);
...
*/
	...
	/* MM-AL-RangeFinderPorting-01*{ */
#if 0
	//dynamic path for attributes group
	rc = sysfs_create_group(&data->input_dev_ps->dev.kobj,
			&stmvl53l0_attr_group);
#else
	//fixed path for attributes group
	rc = sysfs_create_group(data->range_kobj,
			&stmvl53l0_attr_group);
#endif
	/* MM-AL-RangeFinderPorting-01*} */
	if (rc) {
