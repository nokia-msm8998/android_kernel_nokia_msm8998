                                ...
                                char *buf)
{
    ...
    if (numberParameters >= 1)
    {
        ...
        case 0x00:
            ...
            #else
            //Prevent autotune false triggered.
            res = production_test_main(LIMITS_FILE, 1, 0, &todoDefault, INIT_MP);
            #endif
...
static int fts_chip_initialization(struct fts_ts_info *info)
{
    ...
    for (retry = 0; retry <= INIT_FLAG_CNT; retry++)
    {
        ...
        //Prevent autotune triggered by firmware upgrading.
        ret2 = production_test_main(LIMITS_FILE, 1, 0, &todoDefault, INIT_FIELD);
        if (ret2 == OK)
            break;
        initretrycnt++;
...
void stm_fts_fih_cmd(void)
{
	...
	*********************/
    ...
    for(i = 0; i < icommand; i++)
    {
	   ...
	   #if 1
	   res = production_test_main(LIMITS_FILE, 1, 0, &todoDefault, INIT_FIELD);
	   #else
            if (ftsInfo.u32_mpPassFlag!=INIT_MP)
            {
                logError(0, "%s MP Flag not set! \n", tag, res);
