...
int VL53L0_I2CWrite(VL53L0_DEV dev, uint8_t *buff, uint8_t len)
{
	...
	if (dev->bus_type == CCI_BUS) {
		...
		struct msm_camera_i2c_client *client = cci_client_obj->client;
                ...
                if (stmvl53l0_stop_access_on_bus_error != 0) {
                    pr_err("%s:%d stop_access_on_bus_error\n",
                            __func__, __LINE__);
                    return -EIO;
                }

                if (cci_client_obj->power_up != 1) {
                    /* MM-MC-Fix-TOF-deinit-deadlock-00+{ */
                    //pr_err("%s:%d power_up is not 1\n",
                    //        __func__, __LINE__);
                    /* MM-MC-Fix-TOF-deinit-deadlock-00+} */
                    return -EPERM;
                }
...
i2c_read_retry:
		err = client->i2c_func_tbl->i2c_read_seq(client,
							index, buff, len);
		if (err < 0) {
			pr_err("%s:%d failed status=%d, retry %d\n",
					__func__, __LINE__, err, retry);
			if (retry < 3) {
				//msleep(5);
				retry ++;
				goto i2c_read_retry;
