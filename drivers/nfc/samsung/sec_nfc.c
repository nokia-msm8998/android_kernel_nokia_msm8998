...
//add i2c runin test by henry.liu NHK_M690_A01-1100
extern int i2c_devinfo_device_write(char *buf); 


//c01 device not add nfc driver add by henry.liu NHK_M690_A01-1087
...
static int sec_nfc_suspend(struct device *dev)
{
    ...
    int ret = 0;
    /*The clk is enable during suspend causing the rockbottom power too larger. liwei modify 20190716*/
    bool clk_req_high;
    struct sec_nfc_platform_data *pdata = info->pdata;
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	{
		printk("the board id is c01, is not install nfc driver\n");
